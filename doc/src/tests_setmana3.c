...
int main(int argc, char **argv) {
	...
	if (argc != 4) {
        ...
        puts("Us: tests_setmana3 <nom> <n inodes a reservar> \
...
reserveran %d inodes i %d blocs.\n", argv[1], ni, nb);
	...
	/* Comprovar */
	printf("Comprovam que hem alliberat be la primera meitat\n");
...
int comprovar(int n_inodes, int n_blocs) {
	...
	if ((sb.total_inode_count - sb.free_inode_count) != n_inodes ) {
		printf("L'informacio del SB respecte els inodes ocupats es incorrecta");
		printf("hi ha %d i haurien de ser %d\n", 
		(sb.total_inode_count - sb.free_inode_count), n_inodes);
	} else {
		printf("La informacio del SB respecte els inodes ocupats es correcta\n");
	}
