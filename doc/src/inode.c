...
int read_inode(int n_inode, emofs_inode *buf) {
	...
	if (inode_block < 0) {
		...
		printf("read_inode: posicio d'inode negatiu\n");
		return -4;
	}
...
int localize_data(int n_block, emofs_data_loc *loc) {
	...
	int group, pos;
	/* Inicialitzacio del vector de maxims.
	...
	if(!found) {
		puts("Peticio d'un bloc fora de rang");
		return -1;
	}
...
/** Inicialitza un bloc de punters a NULL_POINTER. Funcio interna. */
 ...
 * @inode: punter a l'inode amb que treballam
 * @pos: posicio dins el vector directe
 ...
 * d'inodes pero no corregim el tamany de fitxer.
 * @n_inode: nombre d'inode
 * @l_block: numero de bloc logic.
 * @block: numero de bloc fisic
 * @alloc: si 1 i no existeix el bloc fisic reserva un bloc de dades.
 * @return: 0 si exit. -1 si es volia llegir i no existia. -2 en cas de
