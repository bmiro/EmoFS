/** Simulador de la practica del sistema de fitxers. El programa consisteix en
...
/* Quants de fils escriptors llancarem */
#define PROCESS_NUMBER 100
/* Nombre d'escriptures que fara un fil escriptor */
...
/* Classic foser o enterrador. Compta el nombre de fills morts per saber quan
...
/* Crea el directori dins del sistema de fitxers segons el moment actual. Es de
...
"proceso_n", on n es el seu PID. Alla fara un prueba.dat. Cada 0,1s han de fer
una escriptura "hh:mm:ss Escritura numero i a partir de la posicion j". i es el
nombre d'escriptura i j la posicio, la qual es aleatoria. */
int sim_work() {
	int i = 0;
	char local_path[256];
	char buff[256];
...
int show_work() {
	...
	/* Despres de fer el emofs_dir s'obte una llista de fitxers separats
