...
/** Calcul del nombre d'inodes. Permetrem tenir tants de inodes com blocs de
 * dades aixi el nombre de fitxers petits i no buits sera maxim. Recodem que el
...
int init_superblock(int n_block) {
	...
	/* Comencam a comptar a partir de 0 per tant -1 */
...
/* Totes les operacions d'inodes venen despres d'una lectura de bloc. Cal
...
int alloc_inode(int type) {
	 ...
	 * detectariem que la llista es buida. Si el primer bloc lliure es el
	 * -1.
	 */
...
int free_indirect_inode(int n_indirect, int level, emofs_superblock *sb) {
	...
	if (level == 0) {
		puts("free_indirect_inode: Us incorrecte");
	} else if (level == 1) {
 ...
 * tambe tots els blocs de dades.  Modifica el nombre d'inodes i blocs lliures 
...
int free_inode(int n_inode) {
	...
	/* Situam com a lliure i enllacam la llista */
	inode.type = FREE_INODE;
