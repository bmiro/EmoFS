 ...
 */
	...
	/* S'ha dignorar la primera barra per això i = 1 */
	j = 0;
	k = 0;
	for (i = 1; path[i] != '\0'; i++) {
...
int emofs_extract_dir_entry(char *dir_content, char *filename) {
	 ...
	 * dir_content dins filename llegint al revés i després el
	 * girarem. */
	...
	/* Com que la condició és que haguem trobat un «:» sabem que a
	 * dir_content en queda com a mínim un. El llevam de la sortida.
	 */     
	if (i > 0) {
		dir_content[i] = '\0';
	}
	/* Posam el nom en l'ordre correcte doncs tenim filename a
	 * l'inrevés. Posam també un caràcter null al final. */
	j--;
	for (i = 0; i < (j/2)+1; i++) {
		tmp = filename[i];
