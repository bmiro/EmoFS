...
Tres casos:
	...
	- començar el darrer bloc.
D'aquesta manera és bona idea calcular prèviament a quin cas
pertany.
*/

/** Funció interna per calcular els blocs necessaris per la tasca.
 * Recorda que BLOCK_SIZE determina el nombre de bytes per bloc.
 * @offset: desplaçament inicial.
 * @n_bytes: bytes a llegir.
 * @first: nombre de bloc del primer de tots.
 * @f_offset: desplaçament dins el primer bloc.
 * @l_size: tamany del darrer bloc si n'hi ha més d'un.
 * @return: 	1 sols primer
 *		2 primer i segon
 *		3+ un de començament, final i alguns intermijos
 */
 ...
 * offset = 10 significa que hem de començar per el byte nombre 10 del fitxer.
 * És important actualitzar el tamany del fitxer.  a escriure des del byte
 * número 10.
...
int write_file(int inode, const void *buffer, int offset, int n_bytes) {
	int error, i;
	/* quants de blocs tenim a llegir, nombre de bloc lògic a treballar */
	int how_many = 0;
	int block = 0;
	...
	int bytes_offset = 0; /* Bytes a desplaçar-se */
	int grow_size = 0;
	...
	if (how_many < 1) {
		puts("read_file: error al càlcul de la posició inicial");
		return n_bytes_written;
	}

	/* La idea és igual que a la lectura: fer les tres seccions on:
		Obtenim el bloc lògic.
		Escrivim sobre ell.
		El tornam a disco.
	...
	/* Importantissim: començam a contar des de un ja que
	   que hem llegit el primer bloc. Aquí treballam amb els blocs
	   intermijos. */
	...
	/* Càlcul del tamany de l'inode un cop fetes les modificacions.*/
...
int read_file(int inode, void **buf, int offset, int n_bytes) {
	...
	int b_index = 0;
	int n_block = 0;
	/* Quants de blocs hem de llegir, nombre del primer bloc, desplaçament
	...
	*buf = malloc(n_bytes_readen*sizeof(char));
	...
	/* Recorda que la variable «block» conté el nombre del primer bloc de
	   dades. Anam a llegir blocs sencers, per això descartam el darrer.*/
	for (/*res*/; (how_many -1) > 0 ; how_many--) {
		block++;
	...
	translate_inode(inode, block, &n_block, 0);
	bread(n_block, &tmp_block);
	n_bytes_readen += ((l_size) ? l_size : BLOCK_SIZE); /* Hem llegit el darrer troçet de bloc */
	*buf = realloc(*buf, n_bytes_readen*sizeof(char));
